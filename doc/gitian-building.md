...
*Setup instructions for a Gitian build of JokeCoin Core using a Debian VM or physical system.*
...
Clone the git repositories for JokeCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/clowncrew/jokecoin
...
in the JokeCoin Core repository under 'Fetch and create inputs' to install sources which require
...
To build JokeCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the JokeCoin Core repository.
...
```bash
    ...
    From https://github.com/clowncrew/jokecoin
...
and the jokecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../jokecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/jokecoin
SIGPATH=/some/root/path/jokecoin-detached-sigs

./bin/gbuild --url jokecoin=${BTCPATH},signature=${SIGPATH} ../jokecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/jokecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/jokecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/jokecoin-osx-build.assert
```
...
[jokecoin/gitian.sigs](https://github.com/clowncrew/gitian.sigs/) repository, or if that's not possible to create a pull
request.
