...
# jokecoin_hash
RUN git clone https://github.com/clowncrew/jokecoin_hash
RUN cd jokecoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} jokecoin
RUN useradd -u ${USER_ID} -g jokecoin -s /bin/bash -m -d /jokecoin jokecoin
...
RUN mkdir /jokecoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /jokecoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /jokecoin-src
